...
import { FILTER_CONFIGS, FIXED_OPTIONS, fetchFilterOptions, type ActiveFilters } from '../filters'
...
export default function FilterPanel({ category, filters, onChange, apiBase, tr }: Props) {
      ...
      {open && (
        <div className="filter-panel">
          <div className="filter-panel-header">
            <span className="filter-panel-title">{tr.filters}</span>
            {activeCount > 0 && (
              <button className="filter-reset" onClick={reset}>{tr.filtersClear}</button>
            )}
          </div>
            ...
            {config.map(field => (
              <div key={field.key} className="filter-field">
                ...
                {field.type === 'select' && (
                  ...
                  >
                    <option value="">{tr.filtersAny}</option>
