...
function enricherToPart(p: EnricherProduct, category: Category): Part {
  ...
  return {
    ...
    // CPU
    socket:               get('Сокет'),
    microarchitecture:    get('Мікроархітектура', 'Архітектура процесора'),
    total_cores:          Number(get('Кількість ядер') ?? '') || undefined,
    threads:              Number(get('Кількість потоків') ?? '') || undefined,
    base_clock_ghz:       parseFloat(get('Базова тактова частота', 'Базова частота') ?? '') || undefined,
    boost_clock_ghz:      parseFloat(get('Максимальна тактова частота', 'Максимальна частота') ?? '') || undefined,
    l3_cache_mb:          parseFloat(get('Кеш L3', 'Обсяг кешу L3') ?? '') || undefined,
    tdp_w:                Number(get('Тепловиділення (TDP)', 'TDP') ?? '') || undefined,
    integrated_graphics:  get('Вбудована графіка', 'Вбудоване відео'),
    lithography:          get('Технологічний процес', 'Техпроцес'),
    memory_types:         get("Тип підтримуваної пам'яті", "Підтримка пам'яті"),
    // GPU
    chipset:              get('Чіп (GPU)', 'Модель графічного процесора', 'GPU'),
    memory_gb:            Number(get("Обсяг відеопам'яті", 'Обсяг відеопамяті') ?? '') || undefined,
    memory_type:          get("Тип відеопам'яті", 'Тип відеопамяті'),
    core_base_clock_mhz:  Number(get('Базова частота ядра GPU', 'Базова частота GPU') ?? '') || undefined,
    core_boost_clock_mhz: Number(get('Частота в режимі Boost', 'Частота Boost') ?? '') || undefined,
    memory_bus_bit:       Number(get("Розрядність шини пам'яті", 'Розрядність шини') ?? '') || undefined,
    total_slot_width:     get('Ширина відеокарти (в слотах)', 'Кількість займаних слотів'),
    // Motherboard
    form_factor:          get('Формфактор', 'Форм-фактор'),
    ...
    wifi:                 get('Бездротовий інтерфейс', 'Wi-Fi'),
    bluetooth:            get('Bluetooth'),
    // RAM
    total_capacity_gb:    Number(get("Обсяг пам'яті", 'Загальний обсяг') ?? '') || undefined,
    module_count:         Number(get('Кількість модулів у комплекті') ?? '') || undefined,
    speed_mhz:            Number(get("Частота пам'яті", 'Тактова частота') ?? '') || undefined,
    cas_latency:          Number(get('Затримка CAS (CL)', 'CAS Latency') ?? '') || undefined,
    voltage_v:            parseFloat(get('Напруга живлення') ?? '') || undefined,
    rgb:                  get('RGB підсвічування', 'Підсвічування'),
    profile_support:      get('Підтримка профілів', 'XMP профіль'),
    // PSU
    wattage:              Number(get('Потужність') ?? '') || undefined,
    efficiency_rating:    get('Сертифікат 80 Plus'),
    modular:              get('Підключення кабелів'),
    fanless:              get('Безшумний режим роботи', 'Режим без вентилятора'),
    conn_atx_24pin:       get('ATX 24-контактний'),
    conn_eps_8pin:        get('EPS/CPU 8-контактний', 'CPU 8-pin'),
    conn_pcie_6p2pin:     get('PCIe 6+2-контактний', 'PCI-E 6+2 pin'),
    conn_sata:            get('SATA'),
    // Storage
    storage_type:         get('Тип'),
    interface:            get('Інтерфейс підключення', 'Інтерфейс'),
    capacity_gb:          Number(get('Ємність') ?? '') || undefined,
    nvme:                 get('Підтримка NVMe', 'NVMe'),
    read_speed_mbs:       Number(get('Швидкість читання', 'Макс. швидкість читання') ?? '') || undefined,
    write_speed_mbs:      Number(get('Швидкість запису', 'Макс. швидкість запису') ?? '') || undefined,
    cache_mb:             Number(get('Кеш', "Обсяг кеш-пам'яті") ?? '') || undefined,
    rpm:                  get('Швидкість обертання шпинделя', 'Швидкість обертання'),
  }
}
